...
class GpxImportService {
  Future<GpxImportResult?> pickAndParse() async {
    ...
    final bytes = f.bytes;
    if (bytes == null) {
      throw Exception('Fichier GPX inaccessible (bytes null).');
    }
    ...
    if (pts.length < 2) {
      throw Exception('GPX invalide: aucun tracé exploitable (trkpt/rtept).');
    }

    final fileName = f.name.isNotEmpty ? f.name : 'import.gpx';
    return GpxImportResult(fileName: fileName, points: pts);
  }
}
